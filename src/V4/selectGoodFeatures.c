 ...
 *
 * V4 PHASE 1: GPU-Accelerated Eigenvalue Computation
 * ---------------------------------------------------
 * Added OpenACC parallelization to the most compute-intensive loop:
 * - Eigenvalue computation (lines ~405-438)
 * - Each pixel processes independently (perfect parallelism)
 * - Gradient data copied to GPU once per feature selection
 * - Results copied back to CPU for sorting
 *
 * Key optimizations:
 * - collapse(2) for 2D parallelism across image
 * - Inner window loop kept sequential (too small for parallelism)
 * - Inline eigenvalue calculation (avoid function call overhead)
 * - Index calculation for non-contiguous pointlist storage
 *********************************************************************/
  ...
  selectionMode mode)
{
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    pixels_y = (nrows - bordery - bordery) / (tc->nSkippedPixels + 1);
    pixels_x = (ncols - borderx - borderx) / (tc->nSkippedPixels + 1);
    
    /* For most of the pixels in the image, do ... */
    /* OpenACC: Parallelize eigenvalue computation - each pixel independent */
    float *gradx_data = gradx->data;
    float *grady_data = grady->data;
    
    #pragma acc parallel loop collapse(2) copyin(gradx_data[0:ncols*nrows], grady_data[0:ncols*nrows]) \
                              copyout(pointlist[0:ncols*nrows*3]) \
                              private(gx, gy, gxx, gxy, gyy, xx, yy, val, point_idx)
    for (y = bordery ; y < nrows - bordery ; y += tc->nSkippedPixels + 1) {
      for (x = borderx ; x < ncols - borderx ; x += tc->nSkippedPixels + 1)  {
        ...
        point_idx = ((y - bordery) / (tc->nSkippedPixels + 1)) * pixels_x + 
                    ((x - borderx) / (tc->nSkippedPixels + 1));
        point_idx *= 3;
        ...
        pointlist[point_idx] = x;
        pointlist[point_idx + 1] = y;
        val = (gxx + gyy - sqrtf((gxx - gyy)*(gxx - gyy) + 4*gxy*gxy))/2.0f;
        if (val > limit)  {
          val = (float) limit;
        }
        pointlist[point_idx + 2] = (int) val;
      }
    }
    
    /* Calculate npoints after parallel region */
    npoints = pixels_y * pixels_x;
  }
