 ...
 * trackFeatures.c
 *
 * V4 PHASE 1: CPU-based tracking (GPU offload in convolution layer)
 * ------------------------------------------------------------------
 * The main tracking loop (line ~1368) remains on CPU for Phase 1.
 * 
 * WHY NOT GPU-ACCELERATED YET:
 * 1. Complex inter-pyramid dependencies (coarse to fine tracking)
 * 2. Internal memory allocations in _trackFeature()
 * 3. Conditional branching and early exits
 * 4. Affine consistency checking with dynamic memory
 *
 * GPU BENEFIT ALREADY ACHIEVED:
 * - Pyramid computation (via convolve.c) runs on GPU
 * - Gradient computation (via convolve.c) runs on GPU
 * - These are called multiple times per frame = big speedup
 *
 * PHASE 2/3 TODO:
 * - Refactor _trackFeature() to eliminate dynamic allocations
 * - Create persistent GPU regions for pyramid structures
 * - Parallelize feature tracking loop with proper data management
 *********************************************************************/
